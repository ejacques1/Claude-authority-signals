...
// ============================================
// CLAUDE CODING PROMPT (unchanged)
// ============================================
...
async function claimUncodedUrl(researcherId) {
  const { data: urls, error: fetchError } = await supabase
    ...
    .order('url_citation_count', { ascending: false }) // Process most-cited URLs first
    .limit(1);
...
async function writeToGoogleSheets(rowNumbers, codingResult) {
  ...
  const values = [
    codingResult.source_name,                    // Column E
    codingResult.organization_name,              // Column F
    codingResult.organization_type,              // Column G
    codingResult.institutional_affiliation,      // Column H
    codingResult.author_name,                    // Column I
    codingResult.author_credentials,             // Column J
    codingResult.temporal_date,                  // Column K
    codingResult.content_length_category,        // Column L
    codingResult.medical_review_stated,          // Column M
    codingResult.references_cited,               // Column N
    codingResult.number_of_references,           // Column O
    codingResult.has_schema,                     // Column P
    codingResult.schema_type                     // Column Q
  ];

  let successCount = 0;
  
  // Write to ALL rows that cite this URL
...
export default async function handler(req, res) {
  ...
  const { count = 1, researcher_id = 'vercel-user' } = req.body || {};
  const results = [];

  for (let i = 0; i < Math.min(count, 10); i++) {
    // 1. Claim URL
    const urlRecord = await claimUncodedUrl(researcher_id);
    if (!urlRecord) {
      results.push({ status: 'no_urls_remaining' });
      break;
    }

    console.log(`Processing URL (cited ${urlRecord.url_citation_count} times): ${urlRecord.source_url}`);
    ...
    // 4. Write to ALL Google Sheets rows for this URL
    ...
    results.push({
      ...
      row_numbers: urlRecord.row_numbers,
